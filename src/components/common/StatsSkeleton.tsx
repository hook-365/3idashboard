    ...
    return (
      <div className={`bg-[var(--color-bg-secondary)] rounded-lg p-6 ${className}`}>
        <div className="animate-pulse space-y-4">
          {/* Header */}
          <div className="h-5 w-40 bg-[var(--color-bg-tertiary)] rounded"></div>

          {/* Stats Grid */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="text-center space-y-2">
                <div className="h-8 w-20 bg-[var(--color-bg-tertiary)] rounded mx-auto"></div>
                <div className="h-3 w-24 bg-[var(--color-bg-tertiary)] rounded mx-auto"></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="animate-pulse space-y-6">
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-${columns} gap-4`}>
          {Array.from({ length: columns }).map((_, i) => (
            <div key={i} className="bg-[var(--color-bg-tertiary)] rounded-lg p-4 space-y-3">
              <div className="h-4 w-24 bg-[var(--color-bg-tertiary)] rounded"></div>
              <div className="h-8 w-16 bg-[var(--color-bg-tertiary)] rounded"></div>
  ...
  return (
    <div className={`bg-[var(--color-bg-secondary)] rounded-lg p-6 ${className}`}>
      <div className="animate-pulse space-y-6">
        {/* Header */}
        <div className="space-y-2">
          <div className="h-6 w-56 bg-[var(--color-bg-tertiary)] rounded"></div>
          {showDescription && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="text-center space-y-2">
              <div className="h-8 w-20 bg-[var(--color-bg-tertiary)] rounded mx-auto"></div>
              <div className="h-4 w-28 bg-[var(--color-bg-tertiary)] rounded mx-auto"></div>
            </div>
          ))}
        </div>

        {/* Chart Area */}
        <div className="mt-6 h-64 bg-[var(--color-bg-tertiary)] rounded"></div>
      </div>
    </div>
  );
}
